        ...
        <basic-area>
            ...
            <el-form :inline="true" label-width="80px">
                <search-box>
                    <el-form-item label="减速机分类">
                        <el-select v-model="searchForm.reductionCategoryId" clearable filterable placeholder="请选择" class="st-input-w">
                    ...
                    <el-form-item label="减速机规格">
                        <el-input v-model.trim="searchForm.specification" clearable placeholder="请输入" class="st-input-w"></el-input>
                    </el-form-item>
            ...
            <div>
                ...
                >
                    ...
                    <vxe-table-column field="reductionCategoryName" title="减速机分类" min-width="150"></vxe-table-column>
                    <vxe-table-column field="specification" title="规格" min-width="150"></vxe-table-column>
                    <vxe-table-column field="rotation" title="转动方式" min-width="100">
                        <template v-slot:default="{ row }">
                            <span>{{ RotationList[row.rotation] }}</span>
                        </template>
                    </vxe-table-column>
                    <vxe-table-column field="showProperty" title="显示基础属性" min-width="100">
                        <template v-slot:default="{ row }">
                            <span>{{ row.showProperty | toBool }}</span>
                        </template>
                    </vxe-table-column>
                    <vxe-table-column field="isCover" title="是否含盖板" min-width="100">
                        <template v-slot:default="{ row }">
                            <span v-if="row.showProperty">{{ row.isCover | toBool }}</span>
                            <span v-else>-</span>
                        </template>
                    </vxe-table-column>
                    <vxe-table-column field="isPinboard" title="是否有转接板" min-width="100">
                        <template v-slot:default="{ row }">
                            <span v-if="row.showProperty">{{ row.isPinboard | toBool }}</span>
                            <span v-else>-</span>
                        </template>
                    </vxe-table-column>
                    <vxe-table-column field="isContact" title="是否有接头" min-width="100">
                        <template v-slot:default="{ row }">
                    ...
                    <vxe-table-column field="img" title="缩略图" width="150">
                        <template v-slot:default="{ row }">
                            <div style="line-height: 0px;">
                                <img v-if="row.img" :src="row.img" style="height: 50px;" alt="电机缩略图">
                            </div>
                        </template>
                    </vxe-table-column>
...
<script>
    ...
    import { getReductionList } from "@/api/reduction";
    ...
    export default {
        ...
        methods: {
            ...
            getReductionCategory(){
                getReductionCategoryAll({}).then((res)=>{
                    let data = res.data;
                    if(data.code == 1){
            ...
            showInfo(row){
                this.$router.push({
                    path: "/reduction/reduction-info/" + row.id
                });
            },
            ...
            edit(row){
                this.$router.push({
                    path: "/reduction/reduction-edit/" + row.id
                });
            },
        }
    }
