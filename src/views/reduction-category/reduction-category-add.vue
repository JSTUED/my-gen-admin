                    ...
                    <el-col :lg="{span: 12}" :md="{span:16}">
                        <el-form :model="model" :rules="rules" ref="ruleForm" label-width="100px">
                            ...
                            <el-form-item label="缩略图" prop="img">
                                ...
                                <span class="img-tip-msg">图片大小182 * 182px，透明背景，png格式</span>
                            </el-form-item>
...
<script>
    import { getReductionCategoryById, addReductionCategory, editReductionCategory } from "@/api/reductionCategory";
        ...
        methods: {
            init(){
                ...
                if(this.$route.params.id){
                    this.getReductionCategory(this.$route.params.id);
                    if(this.$route.name == "编辑减速机分类"){
                        this.type = "edit";
                        this.title = "编辑减速机品牌";
                    }
                    if(this.$route.name == "减速机品牌分类"){
                        this.type = "info";
                        this.title = "减速机品牌信息";
                    }
                }else {
            ...
            getReductionCategory(id){
                this.showUpload = false;
                getReductionCategoryById(id).then((res)=>{
            ...
            create(){
                // 创建时调用的方法 提交数据给后台
                this.loading = true;
                addReductionCategory(_.cloneDeep(this.model)).then(res=>{
                    res = res.data;
                    if(res.code == 1){
                        this.$message({
                            type: 'success',
            ...
            update(){
                // 更新时调用的方法 提交数据给后台
                this.loading = true;
                editReductionCategory(_.cloneDeep(this.model)).then(res=>{
                    res = res.data;
                    if(res.code == 1){
                        this.$message({
                            type: 'success',
