                    ...
                    <el-col :lg="{span: 12}" :md="{span:16}">
                        <el-form :model="model" :rules="rules" ref="ruleForm" label-width="100px">
                            ...
                            <el-form-item label="类型" prop="type">
                                <el-select v-model="model.type" clearable placeholder="请选择">
                                    <el-option v-for="(value, key, index) in PropertyTypeList" :key="index" :label="value" :value="key"></el-option>
                                </el-select>
                            </el-form-item>
...
<script>
    import { getPropertyById, addProperty, editProperty } from "@/api/property";
    import { PropertyTypeList, LanguageList, StatusList } from "@/const/constData";
        ...
        methods: {
            init(){
                ...
                if(this.$route.params.id){
                    ...
                    if(this.$route.name == "编辑属性"){
                        this.type = "edit";
                        this.title = "编辑属性";
                    }
                    if(this.$route.name == "属性信息"){
                        this.type = "info";
                        this.title = "属性信息";
                    }
                }else {
            ...
            create(){
                // 创建时调用的方法 提交数据给后台
                this.loading = true;
                addProperty(_.cloneDeep(this.model)).then(res=>{
                    res = res.data;
                    if(res.code == 1){
                        this.$message({
                            type: 'success',
            ...
            update(){
                // 更新时调用的方法 提交数据给后台
                this.loading = true;
                editProperty(_.cloneDeep(this.model)).then(res=>{
                    res = res.data;
                    if(res.code == 1){
                        this.$message({
                            type: 'success',
